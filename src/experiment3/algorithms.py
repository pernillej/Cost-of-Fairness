...
def svm(dataset, fairness_metric, accuracy_metric, C, gamma, keep_features, privileged_threshold,
    ...
    for i in range(len(dataset_orig_test_pred.labels)):
        if dataset_orig_test_pred.features[i][4] == 1.:  # Privileged
            if dataset_orig_test_pred.scores[i] > privileged_threshold:  # Above threshold
                dataset_orig_test_pred.labels[i] = dataset_orig_test_pred.favorable_label
            else:
                dataset_orig_test_pred.labels[i] = dataset_orig_test_pred.unfavorable_label
        else:  # Unprivileged
            if dataset_orig_test_pred.scores[i] > unprivileged_threshold:  # Above threshold
                dataset_orig_test_pred.labels[i] = dataset_orig_test_pred.favorable_label
            else:
                dataset_orig_test_pred.labels[i] = dataset_orig_test_pred.unfavorable_label
...
def svm_reweighing(dataset, fairness_metric, accuracy_metric, C, gamma, keep_features, privileged_threshold,
                   unprivileged_threshold, privileged_groups, unprivileged_groups):
    """
    ...
    for i in range(len(dataset_transf_test_pred.labels)):
        if dataset_transf_test_pred.features[i][4] == 1.:  # Privileged
            if dataset_transf_test_pred.scores[i] > privileged_threshold:  # Above threshold
                ...
                dataset_transf_test_pred.labels[i] = dataset_transf_test_pred.unfavorable_label
        else:  # Unprivileged
            if dataset_transf_test_pred.scores[i] > unprivileged_threshold:  # Above threshold
                dataset_transf_test_pred.labels[i] = dataset_transf_test_pred.favorable_label
            else:
                dataset_transf_test_pred.labels[i] = dataset_transf_test_pred.unfavorable_label
...
def svm_dir(dataset, fairness_metric, accuracy_metric, C, gamma, keep_features, privileged_threshold,
            unprivileged_threshold, privileged_groups, unprivileged_groups):
    """
...
def svm_optimpreproc(dataset, fairness_metric, accuracy_metric, C, gamma, keep_features, privileged_threshold,
                     unprivileged_threshold, privileged_groups, unprivileged_groups):
    """
    ...
    return svm(dataset=dataset, fairness_metric=fairness_metric, accuracy_metric=accuracy_metric, C=C, gamma=gamma,
               keep_features=keep_features, privileged_threshold=privileged_threshold,
               unprivileged_threshold=unprivileged_threshold, privileged_groups=privileged_groups,
