...
from src.nsga2.population import get_classification_threshold, get_C, get_gamma, get_selected_features
from src.metrics import function_name_to_string
from src.experiment3.algorithms import train_svm, test_classifier
...
def svm_experiment(classifier_chromosome, num_generations, population_size, mutation_rate, crossover_rate,
                   chromosome_length, fairness_metric, accuracy_metric, training_data, test_data, privileged_groups,
                   unprivileged_groups, max_iter, svm_seed, name_postfix):
    """
    ...
    svm_classifier, svm_scale = train_svm(training_data=training_data, C=C, gamma=gamma,
                                          keep_features=selected_features, max_iter=max_iter, svm_seed=svm_seed)
    ...
    def evaluation_function(chromosome):
        ...
        if str(chromosome) in FITNESS_SCORES:
            ...
            privileged_threshold = get_classification_threshold(chromosome, 0, 10)
            unprivileged_threshold = get_classification_threshold(chromosome, 10, 20)
    ...
    result_summary = {'name': 'SVM',
                      ...
                      'nsga2_parameters': {'num_generations': num_generations, 'population_size': population_size,
                                           ...
                                           'chromosome_length': chromosome_length}}
    write_result_to_file(result_summary, "svm_" + name_postfix)
