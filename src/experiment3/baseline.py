...
from src.experiment3.algorithms import svm
...
def svm_experiment(num_generations, population_size, mutation_rate, crossover_rate, chromosome_length,
    ...
    def evaluation_function(chromosome):
        ...
        if str(chromosome) in FITNESS_SCORES:
            ...
            selected_features = get_selected_features(chromosome, 40)
            privileged_threshold = get_classification_threshold(chromosome, 30, 35)
            unprivileged_threshold = get_classification_threshold(chromosome, 35, 40)
